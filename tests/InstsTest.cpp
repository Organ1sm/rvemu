...
namespace rvemu
{
    TEST_CASE("RVTests-addi", "Test addi Instructions")
    {
        SECTION("addi instruction with a postive immediate value")
        {
            ...
            auto &cpu        = rvHelper(code, "test_addi1", 1);

            REQUIRE(cpu.getRegs().read(31) == 42);
        }

        SECTION("addi instruction with a negative immediate value")
        {
            std::string code = start + "addi x2, x0, -10 \n";
            auto &cpu        = rvHelper(code, "test_addi2", 1);

            REQUIRE(cpu.getRegs().read(2) == -10);
        }
        ...
        SECTION("addi instruction with a negative immediate value")
        {
            std::string code = start
                               + "addi x2, x0, 10 \n"
                                 "addi x3, x0, -10 \n"
                                 "addi x4, x0, 0\n";
            auto &cpu = rvHelper(code, "test_addi3", 3);

            REQUIRE(cpu.getRegs().read(2) == 10);
            REQUIRE(cpu.getRegs().read(3) == static_cast<uint64_t>(-10));
            REQUIRE(cpu.getRegs().read(4) == 0);
        }
    }
    ...
    TEST_CASE("RVTests-slli", "Test slli Instruction")
    {
        ...
        auto &cpu = rvHelper(code, "test_slli", 2);
        REQUIRE(cpu.getRegs().read(1) == (5 << 3));
    }
    ...
    TEST_CASE("RVTests-slti", "Test slti Instruction")
    {
        SECTION("test slti instruction")
        {
            ...
            auto &cpu = rvHelper(code, "test_slti", 2);
            REQUIRE(cpu.getRegs().read(1) == 1);
        }
        ...
        SECTION("test sltiu instruction")
        {
            ...
            auto &cpu = rvHelper(code, "test_sltiu", 2);
            REQUIRE(cpu.getRegs().read(1) == 1);
        }
        ...
        SECTION("test sltiu2 instruction")
        {
            ...
            auto &cpu = rvHelper(code, "test_sltiu2", 2);
            REQUIRE(cpu.getRegs().read(1) == 0);
        }
    }
    ...
    TEST_CASE("RVTests-xori", "Test xori Instruction")
    {
        ...
        auto &cpu = rvHelper(code, "test_xori", 2);
        REQUIRE(cpu.getRegs().read(1) == 8);
    }
    ...
    TEST_CASE("RVTests-sri", "Test sri Instruction")
    {
        ...
        SECTION("test srai instruction")
        {
            ...
            auto result = static_cast<int64_t>(cpu.getRegs().read(1));
            REQUIRE(cpu.getRegs().read(2) == -16);
            REQUIRE(result == -2);
        }
    }
    ...
    TEST_CASE("RVTests-ori", "Test ori Instruction")
    {
        ...
        REQUIRE(cpu.getRegs().read(1) == (15 | 7));
    }
    ...
    TEST_CASE("RVTests-Fence", "Test Fence Instruction")
    {
        ...
        REQUIRE(cpu.getCurrInst() == DRAM_BASE + 4);
    }
    ...
    TEST_CASE("RVTests-csrrsi", "Test csr-rsi Instruction")
    {
        ...
        {
            std::string code = start + "csrrsi x1, mstatus, 5 \n";    // x1 = mstatus;
                                                                      // mstatus = mstatus | 5;
